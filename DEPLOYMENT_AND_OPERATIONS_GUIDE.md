# 🚀 Agent 65: Complete Deployment & Hybrid Operations Guide
...
### Step 1: Create Free Neon PostgreSQL Database (2 Minutes, No Credit Card)
...
### Step 2: Migrate All Local Student Data to Neon (1 Command)
...
### Step 3: Get Free Groq API Key (1 Minute)
...
### Step 4: Deploy FastAPI Backend on Render (Free Docker)
...
| `FRONTEND_ORIGINS` | `http://localhost:3000` *(Step 5 ke baad hum yahan Vercel link dalenge)* |
| `LOCAL_MODEL_URL` | `http://localhost:11434` *(Tunnel setup ke baad hum isko update karenge)* |
...
### Step 5: Deploy Next.js Frontend on Vercel (1 Click)
...
### Step 6: Link Frontend & Backend CORS (Final Handshake)

1. Render dashboard me jayein ➔ `agent65-api` ➔ **Environment**.
2. `FRONTEND_ORIGINS` variable ko update karein aapke Vercel URL se:
   ```
   FRONTEND_ORIGINS = https://agent65-student-helpdesk.vercel.app,http://localhost:3000
   ```
...
### Situation A: Laptop Band Hai (Ya Aap College Me Ho Bina Laptop Ke)
...
### Situation B: Laptop Khula Hai Aur Aapko Custom Local Model Se Demo Dena Hai
...
### Situation C: Demo Khatam Hone Par Laptop Band Karna
