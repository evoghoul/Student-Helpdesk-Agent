...
export const SecurityDrawer: React.FC<SecurityDrawerProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-end bg-slate-900/40 backdrop-blur-xs">
      <div className="relative w-full max-w-2xl h-full bg-white shadow-2xl border-l border-slate-200 flex flex-col animate-in slide-in-from-right duration-300">
        {/* Drawer Header */}
        ...
        <div className="flex border-b border-slate-200 bg-slate-50 px-5 text-xs font-semibold">
          ...
          >
            <span className="flex items-center gap-1.5">
              ...
              <span>RLS Guardrail Data Flow</span>
            </span>
          </button>
          <button
        ...
        {/* Content Body */}
        <div className="flex-1 overflow-y-auto overscroll-contain p-5 space-y-5 text-xs text-slate-700">
          {activeTab === "architecture" && (
              ...
              <div className="rounded-2xl border border-emerald-200 bg-emerald-50/70 p-4 space-y-2">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-emerald-950 flex items-center gap-1.5">
                    ...
                    <span>Row-Level Access Control (RLAC) Active</span>
                  </span>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50/60 p-4 space-y-3">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    <div
                      key={s.step}
                      className="flex items-start gap-3 rounded-xl bg-white p-2.5 border border-slate-200 shadow-2xs"
                    >
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {INITIAL_SECURITY_LOGS.map((log) => (
                  <div
                    key={log.id}
                    className="rounded-xl border border-slate-200 bg-white p-3 shadow-2xs space-y-1"
                  >
                    <div className="flex items-center justify-between">
                    ...
                    <div className="text-[11px] text-slate-500 flex items-center gap-2">
                      ...
                      <span>•</span>
                      <span className="font-mono text-slate-600">{log.scope}</span>
                    </div>
                    <p className="text-[11px] text-slate-600 italic">{log.details}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="space-y-2">
                {CONNECTED_AGENTS.map((agent) => (
                  <div
                    key={agent.id}
                    className="rounded-xl border border-slate-200 bg-white p-3 shadow-2xs flex items-center justify-between"
                  >
                    <div className="space-y-0.5">
                      ...
                      <div className="text-[11px] text-slate-500">{agent.domain}</div>
