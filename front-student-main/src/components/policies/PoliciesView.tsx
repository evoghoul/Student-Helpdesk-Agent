    ...
    <div className="@container space-y-6">
      ...
      <div className="grid grid-cols-1 @2xl:grid-cols-2 gap-4">
        {filteredPolicies.map((pol) => (
          <div
            key={pol.id}
            className="rounded-2xl border border-slate-200 bg-white p-5 shadow-2xs hover:shadow-sm hover:border-slate-300 transition-all flex flex-col justify-between"
          >
            <div>
              ...
              <h3 className="text-base font-bold text-slate-900 mb-1">{pol.title}</h3>
              <p className="text-xs text-slate-600 leading-relaxed">{pol.shortDescription}</p>
      ...
      {selectedPolicy && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4">
          <div className="relative w-full max-w-2xl max-h-[85vh] overflow-y-auto overscroll-contain rounded-3xl bg-white p-6 shadow-2xl border border-slate-200">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div>
                <span className="rounded-full bg-blue-50 px-2 py-0.5 text-[11px] font-bold text-blue-700">
                  {selectedPolicy.category}
                </span>
                <h3 className="text-lg font-bold text-slate-900 mt-1">{selectedPolicy.title}</h3>
              </div>
              <button
            ...
            <div className="my-4 space-y-4 text-xs text-slate-700 leading-relaxed">
              ...
              <div>
                <h4 className="font-bold text-slate-900 text-sm mb-2">Key Regulatory Articles:</h4>
                <ul className="space-y-2">
                  {selectedPolicy.keyPoints.map((pt, i) => (
                    <li key={i} className="rounded-lg bg-slate-50 p-2.5 border border-slate-100 text-slate-800">
                      {pt}
                    </li>
                  ))}
                </ul>
              ...
              <div>
                <h4 className="font-bold text-slate-900 text-sm mb-1">Senate Ordinance Text:</h4>
                <div className="rounded-xl bg-slate-50 p-3 border border-slate-100 whitespace-pre-line text-slate-600 font-mono text-[11px]">
