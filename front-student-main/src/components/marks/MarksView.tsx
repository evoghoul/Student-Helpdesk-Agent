                ...
                <div key={item.id} className="p-4 hover:bg-slate-50/60 transition-colors">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">Date: {item.date}</p>
                    </div>
                  ...
                  {/* Feedback Snippet */}
                  <div className="mt-2.5 rounded-lg bg-slate-50 p-2.5 border border-slate-100 text-xs text-slate-600 flex items-start gap-2">
                    <span className="text-[11px] font-bold uppercase tracking-wider text-slate-400 shrink-0 mt-0.5">
                      Feedback:
                    </span>
                    <p className="italic text-slate-700">{item.feedback}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="rounded-2xl border border-indigo-200 bg-gradient-to-br from-indigo-50/50 to-white p-5 shadow-2xs space-y-3">
            ...
            <div className="space-y-3">
              {MARKS_DATA.upcomingAssessments.map((upcoming, idx) => (
                <div
                  key={idx}
                  className="rounded-xl border border-indigo-100 bg-white p-3.5 shadow-2xs space-y-1.5"
                >
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-900">{upcoming.title}</span>
                    <span className="text-[11px] font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md">
                      {upcoming.date}
                    </span>
                  </div>
                  <div className="text-xs font-medium text-slate-600">{upcoming.subject}</div>
