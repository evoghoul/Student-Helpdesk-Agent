...
export const CommandBarModal: React.FC<CommandBarModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-slate-900/50 backdrop-blur-xs pt-20 p-4">
      <div className="relative w-full max-w-xl rounded-3xl bg-white shadow-2xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Search Bar Input */}
        <form
          onSubmit={(e) => {
            e.preventDefault();
          ...
          className="flex items-center gap-3 border-b border-slate-100 px-5 py-4 bg-slate-50/50"
        >
          ...
          <kbd className="hidden sm:inline-block rounded border border-slate-200 bg-white px-2 py-0.5 font-mono text-[11px] text-slate-400">
            ESC
          </kbd>
        </form>
          ...
          {defaultSuggestions.map((item, idx) => {
            ...
            return (
              <div
                ...
                className="flex items-center justify-between rounded-xl p-2.5 hover:bg-slate-50 text-slate-700 hover:text-slate-900 transition-colors cursor-pointer"
              >
