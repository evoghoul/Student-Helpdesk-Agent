...
interface AgentChatContextType {
  ...
  modelMode: "fast" | "high";
  setModelMode: (mode: "fast" | "high") => void;
...
export const AgentChatProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [modelMode, setModelModeState] = useState<"fast" | "high">(() => {
    if (typeof window !== "undefined") {
      const saved = localStorage.getItem("agent65_model_mode");
      if (saved === "high" || saved === "fast") return saved;
    }
    return "fast";
  });

  const setModelMode = (mode: "fast" | "high") => {
    setModelModeState(mode);
    if (typeof window !== "undefined") {
      localStorage.setItem("agent65_model_mode", mode);
    }
  };
  ...
  const handleSend = async (textToSend?: string) => {
    ...
    try {
      ...
      const backendResp = await apiClient.sendMessage(query, language, modelMode);
      if (backendResp) {
        setMessages((prev) => [
          ...prev,
          {
